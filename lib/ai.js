...
async function callOpenRouter({prompt,image,model}){
  ...
  const response=await fetch('https://openrouter.ai/api/v1/chat/completions',{
    ...
    body:JSON.stringify({model,messages:[{role:'user',content}],temperature:0.4,max_tokens:1800}),
...
async function generate({prompt,image,audio}){
  ...
  const errors=[];
  // BARKVERSE intentionally uses OpenRouter only. No Google AI API key is required.
  // Vision requests try verified free multimodal models before OpenRouter's capability-aware free router.
