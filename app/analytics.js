...
/* Reliable floating help assistant — exactly one trigger and one panel. */
(() => {
  const boot = () => {
    // Remove every previous help implementation before installing the canonical one.
    document.querySelectorAll('[data-barkverse-help-panel]').forEach(el => el.remove());
    document.querySelectorAll('[data-barkverse-help]').forEach(el => el.remove());
    document.querySelectorAll('button,[role="button"]').forEach(el => {
      const text = `${el.textContent || ''} ${el.getAttribute('aria-label') || ''} ${el.getAttribute('title') || ''}`.toLowerCase().trim();
      const s = getComputedStyle(el); const r = el.getBoundingClientRect();
      if (s.position === 'fixed' && r.width <= 90 && r.height <= 90 && (text === '🐾' || text.includes('help bot') || text.includes('barkverse help'))) el.remove();
    });
    ...
    panel.innerHTML=`<button class="close" type="button" aria-label="Close">×</button><h3></h3><p></p><form><input maxlength="300"><button type="submit"></button></form><div class="answer" aria-live="polite"></div>`;document.body.appendChild(panel);
    ...
    const refresh=()=>{const c=t();title.textContent=c.title;intro.textContent=c.welcome;input.placeholder=c.placeholder;form.querySelector('button[type="submit"]').textContent=c.send;trigger.setAttribute('aria-label',c.title)};refresh();
    ...
    form.addEventListener('submit',e=>{e.preventDefault();const q=input.value.trim();if(!q)return;const c=t();const match=c.answers.find(([re])=>re.test(q));answer.textContent=match?match[1]:(lang()==='hi'?'मैं Camera, Breed, Talk, Voice, Memory, Pawprint, BARKINDER, Games, Language, Privacy और शुरुआत के बारे में बता सकता हूँ।':'I can help with photos, breed estimates, Talk, Voice, Memory, Pawprint, BARKINDER, games, language, privacy, or getting started.');input.value=''});
