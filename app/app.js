...
function renderAvatar() {
  ...
  if (state.imageBase64 && ALLOWED_MIME.has(state.mimeType)) {
    const img = document.createElement('img');
    img.src = `data:${state.mimeType};base64,${state.imageBase64}`;
    img.alt = `${state.dog?.name || 'Dog'} photo`;
    els.dogAvatar.appendChild(img);
...
function renderDog(dog) {
  ...
  els.tagline.textContent = dog.tagline || 'Professional snack inspector.';
  els.chaos.textContent = dog.stats?.chaos ?? 92; els.treats.textContent = dog.stats?.treats ?? 98;
  els.zoomies.textContent = dog.stats?.zoomies ?? 95; els.loyalty.textContent = dog.stats?.loyalty ?? 100;
  ...
  els.breedLabel.textContent = breed.label || 'Unknown / mixed breed';
  els.breedNote.textContent = breed.note || 'Visual estimate only; appearance can overlap across breeds.';
  const confidence = ['high', 'medium', 'low'].includes(breed.confidence) ? breed.confidence : 'low';
  els.breedConfidence.textContent = `${confidence.toUpperCase()} CONFIDENCE`;
  els.breedConfidence.className = `confidence ${confidence}`;
  els.traits.innerHTML = (Array.isArray(dog.traits) ? dog.traits : []).slice(0, 5).map((trait) => `<span class="trait">${escapeHtml(trait)}</span>`).join('');
  renderAvatar();
}
...
els.photo.addEventListener('change', async () => {
  ...
  if (!ALLOWED_MIME.has(file.type) || file.size > MAX_IMAGE_BYTES) {
    els.uploadTitle.textContent = 'Photo not accepted';
    els.uploadHint.textContent = 'Use JPEG, PNG or WebP under 7 MB.';
    els.photo.value = ''; state.imageBase64 = null; return;
  }
  state.mimeType = file.type;
  try {
    const dataUrl = await dataUrlFromFile(file); state.imageBase64 = dataUrl.split(',')[1];
    els.preview.replaceChildren(); const img = document.createElement('img'); img.src = dataUrl; img.alt = 'Uploaded dog'; els.preview.appendChild(img); els.preview.classList.remove('hidden');
    els.uploadTitle.textContent = 'Dog acquired'; els.uploadHint.textContent = 'Barkverse is ready to discover them.';
  } catch { state.imageBase64 = null; els.uploadTitle.textContent = 'Photo could not be read'; els.uploadHint.textContent = 'Please choose another image.'; }
});

els.demo.addEventListener('click', async () => {
  setLoading(els.demo, true, 'Discovering dog…');
  const requestedName = els.nameInput.value.trim().slice(0, 40);
  try {
    ...
    if (!response.ok) throw new Error(dog.error || 'Discovery failed');
    renderDog(dog);
...
els.talkBtn.addEventListener('click', async () => {
  const message = els.talkInput.value.trim(); if (!message) return;
  setLoading(els.talkBtn, true, 'Thinking…'); els.voiceStatus.textContent = '';
  try {
    ...
    const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Talk failed');
    els.dogReply.textContent = data.reply || 'The dog is staring at you thoughtfully.'; els.provider.textContent = `AI engine: ${data.provider || 'BARKVERSE'}`;
...
async function speakDog(text) {
  try {
    const response = await fetch('/api/voice', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: String(text).slice(0, 800) }) });
    if (!response.ok) throw new Error('voice unavailable');
    const blob = await response.blob(); const url = URL.createObjectURL(blob); els.dogAudio.src = url; els.dogAudio.classList.remove('hidden'); await els.dogAudio.play().catch(() => {}); els.voiceStatus.textContent = '🎙️ ElevenLabs dog voice';
  } catch {
    els.voiceStatus.textContent = '🔈 Voice fallback: browser speech';
    if ('speechSynthesis' in window) { window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(text); utterance.rate = 1.05; utterance.pitch = 1.35; window.speechSynthesis.speak(utterance); }
  }
}

els.memoryBtn.addEventListener('click', async () => {
  const memory = els.memoryInput.value.trim(); if (!memory) return;
  ...
  try {
    ...
    const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Memory failed'); state.memory = data;
    els.memoryOutput.innerHTML = `<strong>${escapeHtml(data.title)}</strong><p>${escapeHtml(data.story)}</p><em>${escapeHtml(data.closing)}</em>`; els.pawprintBtn.classList.remove('hidden');
  } catch {
    ...
    state.memory = { title: `${state.dog?.name || 'Your Dog'}'s Little Moment`, story: memory, closing: 'Preserve this moment.' }; els.pawprintBtn.classList.remove('hidden');
  }
...
async function createPawprint() {
  ...
  try {
    const wallet = window.solana; if (!wallet?.isPhantom) throw new Error('Phantom wallet not detected'); await wallet.connect();
    const { Connection, PublicKey, Transaction, TransactionInstruction } = await import('https://esm.sh/@solana/web3.js@1.98.4');
    const connection = new Connection('https://api.devnet.solana.com', 'confirmed'); const publicKey = wallet.publicKey || new PublicKey(wallet.publicKey.toString());
    ...
    const { blockhash, lastValidBlockHeight } = await connection.getLatestBlockhash('confirmed'); const transaction = new Transaction({ recentBlockhash: blockhash, feePayer: publicKey }).add(instruction);
    const signed = await wallet.signAndSendTransaction(transaction); await connection.confirmTransaction({ signature: signed.signature, blockhash, lastValidBlockHeight }, 'confirmed');
    els.pawprintOutput.innerHTML = `🐾 Pawprint preserved on Solana devnet. <a href="https://explorer.solana.com/tx/${encodeURIComponent(signed.signature)}?cluster=devnet" target="_blank" rel="noopener noreferrer">View proof →</a>`;
...
els.observatoryBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Observatory failed'); const s = data.stats || {};
    els.observatoryOutput.innerHTML = [['Dogs connected', Number(s.dogsConnected || 0).toLocaleString()],['Humans investigated', Number(s.humansInvestigated || 0).toLocaleString()],['Average zoomies', `${s.averageZoomies || 0}%`],['Sofa ownership claims', `${s.sofaOwnershipClaims || 0}%`],['Snack inspections', `${s.snackInspections || 0}%`]].map(([label,value]) => `<div><b>${escapeHtml(value)}</b><span>${escapeHtml(label)}</span></div>`).join('');
