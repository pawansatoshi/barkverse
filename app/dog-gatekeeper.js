(() => {
  ...
  const injectedWallets = () => {
    const candidates = [
      ...
      ['Bitget Wallet', window.bitget?.solana],
      ['Brave Wallet', window.braveSolana],
      ['Solana wallet', window.solana]
    ];
    const seen = new Set();
    return candidates.filter(([, wallet]) => {
      if (!wallet || typeof wallet.connect !== 'function' || typeof wallet.signAndSendTransaction !== 'function') return false;
      const key = wallet.publicKey?.toString?.() || wallet;
  ...
  async function getWalletProvider() {
    ...
    if (wallets.length) {
      const [name, wallet] = wallets[0];
      return { wallet, address: wallet.publicKey?.toString?.() || '', source: name };
    }
    if (window.barkverseWalletConnect?.connect) {
      pawprintOutput && (pawprintOutput.textContent = 'No compatible browser wallet detected. Opening the secure WalletConnect wallet chooser…');
      return await window.barkverseWalletConnect.connect();
    }
  ...
  pawprintButton?.addEventListener('click', async (event) => {
    ...
    if (pawprintOutput) pawprintOutput.textContent = 'Connecting wallet and calculating the real Solana devnet fee…';
    try {
      const { wallet, address, source } = await getWalletProvider();
      ...
      const publicKeyValue = wallet.publicKey?.toString?.() || address;
      ...
      let feeInfo = await connection.getFeeForMessage(message, 'confirmed');
      if (feeInfo?.value == null) {
        await new Promise(resolve => setTimeout(resolve, 350));
        feeInfo = await connection.getFeeForMessage(message, 'confirmed');
      }
      ...
      if (!tx) throw new Error('Transaction was confirmed but its details are not available yet. Please refresh and verify it on Solana Explorer.');
      ...
      if (pawprintOutput) pawprintOutput.innerHTML = `🐾 Pawprint preserved on Solana devnet · actual network fee ${(actualLamports / 1e9).toFixed(9)} SOL · <a href="https://explorer.solana.com/tx/${encodeURIComponent(signature)}?cluster=devnet" target="_blank" rel="noopener noreferrer">View transaction →</a>`;
    } catch (error) {
